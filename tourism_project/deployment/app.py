...
# This list must match the order and names of features generated by prep.py
FINAL_FEATURE_COLUMNS = [
    'CityTier', 'DurationOfPitch', 'MonthlyIncome', 'NumberOfFollowups',
    'NumberOfPersonVisiting', 'NumberOfTrips', 'PitchSatisfactionScore',
    'PreferredPropertyStar', 'Passport', 'OwnCar', 'NumberOfChildrenVisiting',
    'IncomePerPerson', 'HasChildren', 'IsMajorCity', 'PitchEffectiveness',
    'TypeofContact_Company Invited', 'TypeofContact_Self Inquiry',
    'Occupation_Freelancer', 'Occupation_Salaried', 'Occupation_Small Business', 'Occupation_Unemployed',
    'Gender_Female', 'Gender_Male',
    'MaritalStatus_Divorced', 'MaritalStatus_Married', 'MaritalStatus_Single',
    'Designation_Executive', 'Designation_Manager', 'Designation_Senior Manager', 'Designation_VP',
    'ProductPitched_Basic', 'ProductPitched_Deluxe', 'ProductPitched_Platinum',
...
with st.sidebar:
    ...
    designation = st.selectbox("Designation", ['Executive', 'Manager', 'Senior Manager', 'VP'], index=0)
    gender = st.selectbox("Gender", ['Male', 'Female'], index=0)
    marital_status = st.selectbox("Marital Status", ['Single', 'Married', 'Divorced'], index=0)
    occupation = st.selectbox("Occupation", ['Salaried', 'Freelancer', 'Small Business', 'Unemployed'], index=0)
    product_pitched = st.selectbox("Product Pitched", ['Basic', 'Deluxe', 'Platinum', 'Standard', 'Super Deluxe'], index=3)
    type_of_contact = st.selectbox("Type of Contact", ['Company Invited', 'Self Inquiry'], index=0)
...
def predict_purchase(
    ...
    if f'TypeofContact_{type_of_contact}' in input_data: input_data[f'TypeofContact_{type_of_contact}'] = 1.0

    # AgeGroup encoding (manual based on bins used in prep.py)
    age_group = ''
    if age < 18: age_group = '<18'
    elif 18 <= age <= 30: age_group = '18-30'
    elif 31 <= age <= 45: age_group = '31-45'
    elif 46 <= age <= 60: age_group = '46-60'
    else: age_group = '60+'
    if f'AgeGroup_{age_group}' in input_data: input_data[f'AgeGroup_{age_group}'] = 1.0
