...
# we will create a temporary directory to store the downloaded raw data
local_raw_data_dir = "./hf_downloaded_raw_data"
os.makedirs(local_raw_data_dir, exist_ok=True)

# Download and load the main dataset from hugging face into our local directory 
tourism_csv = hf_hub_download(
    repo_id=REPO_ID,
...
df.drop(columns=['CustomerID'], inplace=True)
 ...
 #The 'Unnamed: 0' column often appears in datasets when a Pandas DataFrame has been saved to a CSV file with the index=True argument (which is the default behavior for to_csv), and then later read back into a DataFrame without specifying index_col=0. Essentially, Pandas writes the DataFrame's index as a regular column with no name, which then gets assigned 'Unnamed: 0' upon reloading.
...
print("Handling Missing values if present for both numeric and categorical columns.")
